...
└── docs/
    ...
    └── progress.md           # Running build log (canonical "where we are")
```
...
firebase use dev | prod                   # switch environments
...
Two projects: `dutch-lanka-dev` and `dutch-lanka-prod`. Switch with `firebase use <alias>`. Each Flutter flavor (`dev`, `prod`) binds to one project via `firebase_options_<flavor>.dart` (generated by `flutterfire configure`). A staging environment was deliberately dropped — easy to add later by re-running `flutterfire configure --project=dutch-lanka-staging` and adding a third flavor block to gradle.
